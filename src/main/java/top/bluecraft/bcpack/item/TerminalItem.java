...
package top.bluecraft.bcpack.item;

import top.bluecraft.bcpack.world.inventory.LoadGunsGuiMenu;
import top.bluecraft.bcpack.item.inventory.TerminalInventoryCapability;
...
public class TerminalItem extends Item {
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new LoadGunsGuiMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new TerminalInventoryCapability();
	}
